...
class BuildGeometry_v4:
        ...
        for n in range(niter):
            for sub in range(nsubs):
                ...
                if Flag:
                    if np.any(psf != 0):
                        sensImageSubs[:, sub] = (
                            self.gaussFilter(sensImage.copy(), psf) + 1e-5
                        )
                    else:
                ...
                img = img_em.copy()
                img_reg = (
                    1 / (2 * wj) * prior.imCropUndo((W * prior.Div(img)).sum(axis=1))
                )
    ...
    def mrMAPEM2DBatch(
        self, prompts, AN, mrImg, RS=None, beta=1, niters=15, nsubs=14, psf=0
    ):
        from geometry.Prior import Prior
