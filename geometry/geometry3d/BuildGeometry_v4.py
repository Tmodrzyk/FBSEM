...
class BuildGeometry_v4:
    ...
    def __computeGantryInfo(self, g):
        ...
        self.scanner.nCrystalsPerBlock = (
            self.scanner.nPhysCrystalsPerBlock + self.scanner.useVirtualCrystal
        )
        self.scanner.nCrystalsPerRing = (
            self.scanner.nBlockPerRing * self.scanner.nCrystalsPerBlock
        )
        if self.scanner.model_number == 1104:
            self.scanner.nCrystalRings = (
                self.scanner.nBlockRings * self.scanner.nPhysCrystalsPerBlock
                + (self.scanner.nBlockRings - 1) * self.scanner.useVirtualCrystal
            )
        elif self.scanner.model_number == 2008:
            self.scanner.nCrystalRings = (
                self.scanner.nBlockRings * self.scanner.nPhysCrystalsPerBlock
            )
        self.scanner.effDetectorRadiusCm = (
            self.scanner.detectorRadiusCm + self.scanner.LORDOIcm
        )
        self.scanner.isTof = self.sinogram.nTofBins > 1
        self.scanner.TofBinWidthNsec = (
            self.scanner.coinciWindowWidthNsec / self.sinogram.nTofBins
        )
        self.scanner.planeSepCm = self.scanner.zCrystalDimCm / 2.0
        self.sinogram.nAngularBins = (
            self.scanner.nCrystalsPerRing // 2 // self.sinogram.nMash
        )
        self.image.matrixSize = [
            self.sinogram.nRadialBins,
            self.sinogram.nRadialBins,
            2 * self.scanner.nCrystalRings - 1,
        ]
        self.image.voxelSizeCm = [
            self.scanner.xCrystalDimCm / 2.0,
            self.scanner.xCrystalDimCm / 2.0,
            self.scanner.planeSepCm,
        ]
        self.is3d = False
    ...
    def __load_gantry_dict(self, g):
        ...
        self.sinogram.nRadialBins = g["nRadialBins"] - int(
            np.ceil(g["nRadialBins"] * (self.sinogram.radialBinCropfactor) / 2.0) * 2
        )
    ...
    def setApirlMmrEngine(
        self, binPath=None, temPath=None, gpu=True, multiprocess=True
    ):
        if binPath is None:
            binPath = (
                r"C:\MatlabWorkSpace\apirl-tags\APIRL1.3.3_win64_cuda8.0_sm35\build\bin"
            )
        if temPath is None:
            temPath = os.getcwd()
    ...
    def buildMichelogram(self):
        a = np.transpose(
            np.arange(1, self.scanner.nCrystalRings**2 + 1).reshape(
                self.scanner.nCrystalRings, self.scanner.nCrystalRings
            )
        )
        b = np.arange(
            -1 * self.scanner.maxRingDiff, self.scanner.maxRingDiff + 1
        ).reshape(self.sinogram.nSegments, self.sinogram.span)
        ...
        for j in range(self.sinogram.nSegments):
            ...
            if j == direction and isodd:
                ...
                c = 1
                k = 0
            oddPlanes, maxNumberOfPlanesPerSeg[j, 0] = self.__zero_pad(
                diagonalsPerSegment[k::2]
            )  # Odd planes
            evenPlanes, maxNumberOfPlanesPerSeg[j, 1] = self.__zero_pad(
                diagonalsPerSegment[c::2]
            )  # Even planes
            OddEvenPlanesPerSegment = np.empty(
                (np.sum(maxNumberOfPlanesPerSeg[j, :]),), dtype=object
            )
        ...
        self.sinogram.totalNumberOfSinogramPlanes = np.sum(
            self.sinogram.numberOfPlanesPerSeg
        )
        return Segments
    ...
    def plotMichelogram(self, showRingNumber=0):
        ...
        ax.set_xticks(np.arange(-0.5, self.scanner.nCrystalRings, 1), minor=True)
        ax.set_yticks(np.arange(-0.5, self.scanner.nCrystalRings, 1), minor=True)
        ax.grid(which="minor", color="k", linestyle="-", linewidth=2)
        plt.title(
            "Michelogram: Span = {}, nSegments = {}".format(
                self.sinogram.span, self.sinogram.nSegments
            ),
            fontsize=18,
        )
        plt.tight_layout()
    ...
    def LorsAxialCoor(self):
        ...
        Segments = self.buildMichelogram()
        z_axis = self.scanner.zCrystalDimCm * np.arange(
            -(self.scanner.nCrystalRings - 1) / 2,
    ...
    def plotLorsAxialCoor(self, plotSeparateSegmentsToo=0):
        ...
        for j in range(self.sinogram.nSegments):
            for i in range(len(axialCoorPerSeg[j])):
                plt.plot(
                    axialCoorPerSeg[j][i, 0:2],
                    axialCoorPerSeg[j][i, 2:4],
                    color="green",
                    linestyle="solid",
                )
        plt.plot(
        ...
        if plotSeparateSegmentsToo == 1:
            ...
            for j in range(self.sinogram.nSegments):
                ...
                for i in range(len(axialCoorPerSeg[ii])):
                    plt.plot(
                        axialCoorPerSeg[ii][i, 0:2],
                        axialCoorPerSeg[ii][i, 2:4],
                        color="green",
                        linestyle="solid",
                    )
    ...
    def LorsTransaxialCoor(self):
        ...
        startXtal = (
            self.scanner.nCrystalsPerRing - self.sinogram.nRadialBins
        ) // 4  # mmr = 40, mct=68 for radialBinCropfactor = 1
        ...
        idx = np.arange(
            ...
            self.scanner.nCrystalsPerRing + self.scanner.nPhysCrystalsPerBlock + 1,
            self.scanner.nPhysCrystalsPerBlock + 1,
        )
        ...
        increment[0::2, 1] = np.arange(
            self.scanner.nCrystalsPerRing / 2 + 1, self.scanner.nCrystalsPerRing + 1
        )
        increment[1::2, 1] = increment[0::2, 1]

        halfNumberOfRadialBins = (
            self.sinogram.nRadialBins
        ) // 2 + 1  # Before interleaving
        ...
        for ii in range(self.scanner.nCrystalsPerRing):
            s1 = (
                startXtal + np.arange(0, halfNumberOfRadialBins, dtype="int16")
            ) - increment[ii, 0]
            s2 = (
                startXtal + np.arange(0, halfNumberOfRadialBins, dtype="int16")
            ) - increment[ii, 1]
        ...
        if 0:  # test rotation
            ...
            for ii in range(0, self.scanner.nCrystalsPerRing, 1):
                ...
                for j in range(halfNumberOfRadialBins):
                    ...
                    plt.axis("equal")
                    plt.xlim(
                        (-self.scanner.transaxialFovCm, self.scanner.transaxialFovCm)
                    )
                    plt.ylim(
                        (-self.scanner.transaxialFovCm, self.scanner.transaxialFovCm)
                    )
        ...
        # Interleaving
        xy1 = np.zeros(
            [self.scanner.nCrystalsPerRing // 2, self.sinogram.nRadialBins, 2]
        )
        xy2 = np.zeros(
            [self.scanner.nCrystalsPerRing // 2, self.sinogram.nRadialBins, 2]
        )
        gaps = np.zeros(
            [self.scanner.nCrystalsPerRing // 2, self.sinogram.nRadialBins],
            dtype="int16",
        )  #
        ...
        for i in range(self.scanner.nCrystalsPerRing // 2):
            ...
            xy1[i, 1 : self.sinogram.nRadialBins : 2, :] = (
                P1[0:-1, :] + P1[1::, :]
            ) / 2
            xy2[i, 0 : self.sinogram.nRadialBins : 2, :] = P2[0:-1, :]
            xy2[i, 1 : self.sinogram.nRadialBins : 2, :] = (
                P2[0:-1, :] + P2[1::, :]
            ) / 2
        ...
        if self.sinogram.nMash == 2:
            ...
            gap = np.zeros(
                [self.sinogram.nAngularBins, self.sinogram.nRadialBins], dtype="int16"
            )
    ...
    def plotLorsTransaxialCoor(self):
        ...
        for i in range(self.sinogram.nAngularBins // 4):
            ...
            plt.plot(
                np.array([xy1[i, idx, 0], xy2[i, idx, 0]]),
    ...
    def Lors3DEndPointCoor(self, reduce4symmetries=0):
        ...
        for j in range(self.sinogram.nSegments):
            for i in range(self.sinogram.numberOfPlanesPerSeg[j]):
                z1 = axialCoorPerSeg[j][i, 0] * np.ones(
                    [self.sinogram.nAngularBins, self.sinogram.nRadialBins]
                )
                z2 = axialCoorPerSeg[j][i, 1] * np.ones(
                    [self.sinogram.nAngularBins, self.sinogram.nRadialBins]
                )
        ...
        xyz2 = 0 * xyz02
        for i in range(self.sinogram.nSegments):
            xyz1[:, :, :, newPlaneRange[i, 0] : newPlaneRange[i, 1]] = xyz01[
                :, :, :, planeRange[o[i], 0] : planeRange[o[i], 1]
            ]
            xyz2[:, :, :, newPlaneRange[i, 0] : newPlaneRange[i, 1]] = xyz02[
                :, :, :, planeRange[o[i], 0] : planeRange[o[i], 1]
            ]
        if reduce4symmetries == 1:
            self.calculateAxialSymmetries()
            xyz1 = xyz1[
                0 : self.sinogram.nAngularBins // 2,
                :,
                :,
                self.sinogram.uniqueAxialPlanes - 1,
            ]
            xyz2 = xyz2[
                0 : self.sinogram.nAngularBins // 2,
                :,
    ...
    def calculateAxialSymmetries(self):
        ...
        for i in range(len(symID)):
            if np.any((i + 1) == idx):
                ii = np.concatenate(
                    [np.arange(P[i, 0], P[i, 1] + 1), np.arange(P[i, 2], P[i, 3] + 1)]
                )
                Ax[ii - 1] = symID[i]
        ...
        for i in range(self.sinogram.totalNumberOfSinogramPlanes):
            if mirror[i] == 1:
                ...
                x = P[j, 0:3]
                offset[i] = (
                    (self.image.matrixSize[2] - 1) - (x[1] - x[0]) + ((i + 1) - x[2])
                )
        planeMirrorTranslation = np.zeros(
            [self.sinogram.totalNumberOfSinogramPlanes, 3], dtype="int16"
        )
    ...
    def calculateSystemMatrixPerPlane(self, xyz1, xyz2, I, reconFovRadious=None):
        ...
        sMatrix = np.zeros(
            (self.sinogram.nAngularBins // 2, self.sinogram.nRadialBins), dtype=object
        )
        if self.scanner.isTof:
            from scipy.stats import norm

            tofBinBoundaries = np.linspace(
                -self.scanner.coinciWindowWidthNsec / 2,
                self.scanner.coinciWindowWidthNsec / 2,
                self.sinogram.nTofBins + 1,
            )
            sigma = self.scanner.tofResolutionNsec / np.sqrt(np.log(256))
            tofMatrix = np.zeros(
                (self.sinogram.nAngularBins // 2, self.sinogram.nRadialBins),
                dtype=object,
            )
        ...
        for ang in range(self.sinogram.nAngularBins // 2):
            for rad in range(self.sinogram.nRadialBins):
                ...
                if amin < amax:
                    ax = paramInterSectPoint(
                        p1x, p2x, amin, axmin, amax, axmax, tx, bx, dx, Nx
                    )
                    ay = paramInterSectPoint(
                        p1y, p2y, amin, aymin, amax, aymax, ty, by, dy, Ny
                    )
                    az = paramInterSectPoint(
                        p1z, p2z, amin, azmin, amax, azmax, tz, bz, dz, Nz
                    )
                    ...
                    LorIntersectionLength = (
                        (a[k + 1] - a[k]) * np.sqrt(tx**2 + ty**2 + tz**2) * 1e4 / dx
                    )  # normaized by pixel size

                    M = (
                        np.stack([im, jm, km, LorIntersectionLength])
                        .transpose()
                        .astype("int16")
                    )
                    ...
                    if (
                        reconFovRadious != 0
                    ):  # remove LOR interactions out of reduced reconstruction FOV
                        IdsToKeep = (
                            vCenter_y[M[:, 0]] ** 2 + vCenter_x[M[:, 1]] ** 2
                        ) < (reconFovRadious) ** 2
                        M = M[IdsToKeep, :]
                    ...
                    if M.size != 0:
                        ...
                        if self.scanner.isTof:
                            ...
                            endPoint1 = np.tile(
                                np.array([p1x, p1y, p1z]), (nEmiPoint, 1)
                            )
                            endPoint2 = np.tile(
                                np.array([p2x, p2y, p2z]), (nEmiPoint, 1)
                            )
                            dL = np.sqrt(
                                np.sum((endPoint2 - VoxelCenters) ** 2, axis=1)
                            ) - np.sqrt(np.sum((endPoint1 - VoxelCenters) ** 2, axis=1))
    ...
    def buildSystemMatrixUsingSymmetries(
        self, save_dir=None, reconFovRadious=None, is3d=False, ncores=1
    ):
        import os
        ...
        if ncores == 1:
            for i in range(N):
                print(i)
                geoMatrix, tofMatrix = self.calculateSystemMatrixPerPlane(
                    xyz1, xyz2, i, reconFovRadious
                )
    ...
    def loadSystemMatrix(self, save_dir, is3d=False, tof=True, reconFovRadious=None):
        ...
        save_dir = save_dir + self.engine.bar
        tic = time.time()
        if (not is3d and not os.path.isfile(save_dir + "geoMatrix-0.npy")) or (
            is3d and not os.path.isfile(save_dir + "geoMatrix-1.npy")
        ):
            self.buildSystemMatrixUsingSymmetries(
                save_dir, is3d=is3d, reconFovRadious=reconFovRadious
            )
            ...
            for i in range(N):
                ...
                if tof and self.scanner.isTof:
                    self.tofMatrix.append(
                        np.load(save_dir + "tofMatrix-" + str(i) + ".npy")
                    )
        else:
            self.buildMichelogram()
            if self.sinogram.radialBinCropfactor != param["radialBinCropfactor"]:
                raise ValueError(
                    f"Current radialBinCropfactor does't match that '{save_dir+'geoMatrix-0.npy'}', choose a different path to recompute system matrix"
                )
            self.geoMatrix.append(
                np.load(save_dir + "geoMatrix-0.npy", allow_pickle=True)
            )
            if tof and self.scanner.isTof:
                self.tofMatrix = []
                self.tofMatrix.append(
                    np.load(save_dir + "tofMatrix-0.npy", allow_pickle=True)
                )
    ...
    def buildPhantom(self, model=0, display=False):
        if model == 0:  # shepp-logan like phantom
            x = np.arange(
                -self.image.matrixSize[0] // 2, self.image.matrixSize[0] // 2, 1
            )
            y = np.arange(
                -self.image.matrixSize[1] // 2, self.image.matrixSize[1] // 2, 1
            )
            z = np.arange(
                -self.image.matrixSize[2] // 2, self.image.matrixSize[2] // 2, 1
            )
        ...
        if display:
            ...
            (
                plt.subplot(2, 2, 1),
                plt.imshow(img[:, :, slices[0]]),
                plt.title("Slice: {}".format(str(slices[0])), fontsize=15),
            )
            (
                plt.subplot(2, 2, 2),
                plt.imshow(img[:, :, slices[1]]),
                plt.title("Slice: {}".format(str(slices[1])), fontsize=15),
            )
            (
                plt.subplot(2, 2, 3),
                plt.imshow(img[:, :, slices[2]]),
                plt.title("Slice: {}".format(str(slices[2])), fontsize=15),
            )
            (
                plt.subplot(2, 2, 4),
                plt.imshow(img[:, :, slices[3]]),
                plt.title("Slice: {}".format(str(slices[3])), fontsize=15),
            )
            plt.show()
        return img
    ...
    def bit_reverse(self, mm):
        ...
        bin2dec_reverse = lambda x: np.array(
            [int(x[i][::-1], 2) for i in range(len(x))]
        )
    ...
    def forwardProject3D(self, img3d, tof=False, psf=0):
        ...
        for i in range(len(self.sinogram.uniqueAxialPlanes)):
            allPlanes.append(
                np.nonzero(self.sinogram.planeMirrorTranslation[:, 0] == i + 1)[0]
            )
        ...
        if tof:
            dims.append(self.sinogram.nTofBins)
            ...
            for j in range(self.sinogram.nRadialBins):
                for p in range(nUniqueAxialPlanes):
                    ...
                    if not np.isscalar(M0):
                        ...
                        idx1 = (M[:, 0] + M[:, 1] * matrixSize[0]).reshape(
                            -1, 1
                        ) + idxAxial
                        idx2 = (
                            M[:, 1] + matrixSize[0] * (matrixSize[0] - 1 - M[:, 0])
                        ).reshape(-1, 1) + idxAxial
                        #                        if tof:
    ...
    def MLEM3D_python(
        self, prompts, img=None, RS=None, niter=100, AN=None, tof=False, psf=0
    ):
        import time
                ...
                for j in range(self.sinogram.nRadialBins):
                    for p in range(nUniqueAxialPlanes):
                        ...
                        if not np.isscalar(M0):
                            ...
                            if tof:
                                W = self.tofMatrix[p][i, j] / 1e4
                                f1 = AN[j, i, allPlanes[p]].reshape(-1, 1) * (
                                    prompts[j, i, allPlanes[p], :]
                                    / (
                                        (G.reshape(-1, 1) * imgOld[idx1]).T.dot(W)
                                        + RS[j, i, allPlanes[p], :]
                                        + 1e-5
                                    )
                                )
                                f2 = AN[j, i + q, allPlanes[p]].reshape(-1, 1) * (
                                    prompts[j, i + q, allPlanes[p], :]
                                    / (
                                        (G.reshape(-1, 1) * imgOld[idx2]).T.dot(W)
                                        + RS[j, i + q, allPlanes[p], :]
                                        + 1e-5
                                    )
                                )
                                ...
                                f1 = AN[j, i, allPlanes[p]] * (
                                    prompts[j, i, allPlanes[p]]
                                    / (
                                        AN[j, i, allPlanes[p]] * G.dot(imgOld[idx1])
                                        + RS[j, i, allPlanes[p]]
                                        + 1e-5
                                    )
                                )
                                f2 = AN[j, i + q, allPlanes[p]] * (
                                    prompts[j, i + q, allPlanes[p]]
                                    / (
                                        AN[j, i + q, allPlanes[p]] * G.dot(imgOld[idx2])
                                        + RS[j, i + q, allPlanes[p]]
                                        + 1e-5
                                    )
                                )
                                backProjImage[idx1] += G.reshape(-1, 1).dot(
                                    f1.reshape(1, -1)
                                )
                                backProjImage[idx2] += G.reshape(-1, 1).dot(
                                    f2.reshape(1, -1)
                                )
                            if Flag:
                                if tof:
                                    GW = G * np.sum(W, axis=1)
                                    sensImage[idx1] += GW.reshape(-1, 1).dot(
                                        AN[j, i, allPlanes[p]].reshape(1, -1)
                                    )
                                    sensImage[idx2] += GW.reshape(-1, 1).dot(
                                        AN[j, i + q, allPlanes[p]].reshape(1, -1)
                                    )
                                else:
                                    sensImage[idx1] += G.reshape(-1, 1).dot(
                                        AN[j, i, allPlanes[p]].reshape(1, -1)
                                    )
                                    sensImage[idx2] += G.reshape(-1, 1).dot(
                                        AN[j, i + q, allPlanes[p]].reshape(1, -1)
                                    )
    ...
    def OSMAPEM2D_DePierro(
        ...
        img = img.flatten("F")
        sensImage = np.zeros_like(img)
        sensImageSubs = np.zeros(
            (np.prod(self.image.matrixSize[:2]), nsubs), dtype="float"
        )
        ...
        if AN is None:
            AN = np.ones(
                [self.sinogram.nRadialBins, self.sinogram.nAngularBins], dtype="float"
            )
        matrixSize = self.image.matrixSize
        if prior is None:
        ...
        for n in range(niter):
            for sub in range(nsubs):
                ...
                img = img_em
                img_reg = (
                    1 / (2 * wj) * prior.imCropUndo((W * prior.Div(imgOld)).sum(axis=1))
                )
                ...
                img = (
                    ...
                    * img_em
                    / (
                        np.sqrt((1 - betaj * img_reg) ** 2 + 4 * betaj * img_em)
                        + (1 - betaj * img_reg)
                        + 1e-5
                    )
                )
            ...
            if display:
                (
                    plt.imshow(
                        img.reshape(matrixSize[:2], order="F"), vmin=0, vmax=3.5
                    ),
                    plt.title("img", fontsize=15),
                )
                plt.pause(0.15)
            Flag = False
            sensImage = 0
    ...
    def forwardDivideBackwardBatch2D(
        self, imgb, prompts, RS=None, AN=None, nsubs=1, subset_i=0, tof=False, psf=0
    ):
        ...
        if np.any(psf != 0):
            for b in range(batch_size):
                backProjImage[b, :] = (
                    self.gaussFilter(backProjImage[b, :], psf) * self.mask_fov()
                )

        # print(f'{batch_size} batches forwad-backprojected in: {(time.time()-tic)/60:.3f} min.')
        return backProjImage

    def forwardBackwardBatch2D(
        self, imgb, RS=None, AN=None, nsubs=1, subset_i=0, tof=False, psf=0
    ):
        # 3D sinodata dimensions: (batch_size, nRadialBins, nAngularBins, totalNumberOfSinogramPlanes, nTofBins)
        # 3D images dimnesions: (batch_size,nColums,nRows,nSlices)
        # import time
        ...
        backProjImage = np.reshape(
            backProjImage, [batch_size, matrixSize[0], matrixSize[1]], order="F"
        )
        if batch_size == 1:
                ...
                for j in range(self.sinogram.nRadialBins):  # nTrimmedRadialBins:
                    ...
                    if not np.isscalar(M0):
                        ...
                        if tof:
                            W = self.tofMatrix[0][i, j] / 1e4
                            for b in range(batch_size):
                                backProjImage[b, idx1, s] += (
                                    G * AN[b, j, i] * W.dot(prompts[b, j, i, :])
                                )
                                backProjImage[b, idx2, s] += (
                                    G * AN[b, j, i + q] * W.dot(prompts[b, j, i + q, :])
                                )
                        else:
                            for b in range(batch_size):
                                backProjImage[b, idx1, s] += (
                                    G * AN[b, j, i] * (prompts[b, j, i])
                                )
                                backProjImage[b, idx2, s] += (
                                    G * AN[b, j, i + q] * (prompts[b, j, i + q])
                                )
            if np.any(psf != 0):
                for b in range(batch_size):
                    backProjImage[b, :, s] = (
                        self.gaussFilter(backProjImage[b, :, s], psf) * self.mask_fov()
                    )

        backProjImage = np.reshape(
            backProjImage, [batch_size, matrixSize[0], matrixSize[1], nsubs], order="F"
        )
        # if nsubs==1:
        #     backProjImage = backProjImage[:,:,:,0]
        if batch_size == 1:
    ...
    def mask_fov(self, reconFovRadious=None):
        if self.fov_mask is None:
            if reconFovRadious is not None:
                ...
                reconRadious = (
                    self.sinogram.nRadialBins * self.scanner.xCrystalDimCm / 4 * 0.96
                )
            x = self.image.voxelSizeCm[0] * np.arange(
                -self.image.matrixSize[0] // 2, self.image.matrixSize[0] // 2, 1
            )
            y = self.image.voxelSizeCm[1] * np.arange(
                -self.image.matrixSize[1] // 2, self.image.matrixSize[1] // 2, 1
            )
    ...
    def OSEM3D_python(
        self, prompts, img=None, RS=None, niter=100, nsubs=1, AN=None, psf=0
    ):
        import time
                    ...
                    for j in range(self.sinogram.nRadialBins):
                        for p in range(nUniqueAxialPlanes):  #
                            ...
                            if not np.isscalar(M0):
                                ...
                                idx2 = (
                                    M[:, 1]
                                    + matrixSize[0] * (matrixSize[0] - 1 - M[:, 0])
                                ).reshape(-1, 1) + idxAxial
    ...
    ******************************************************************************************************************
    ...
    def createConfigFile(
        ...
        f.write(
            "siddon number of samples on the detector := 1\n"
    ...
    def write_to_apirl(self, flname, data=None, sino_mode=False):
        ...
        if sino_mode:
            ...
            f.write(
                "minimum ring difference per segment := {  -5, 6, -16, 17, -27, 28, -38, 39, -49, 50, -60 }\n"
    ...
    def simulateSinogramData(
                ...
                for b in range(batch_size):
                    for i in range(self.sinogram.totalNumberOfSinogramPlanes):
                        NF[b, :, :, i] = gaps * np.exp(
                            -1
                            * np.random.rand(
                                self.sinogram.nRadialBins, self.sinogram.nAngularBins
                            )
                        )
            else:
                for b in range(batch_size):
        ...
        if batch_size > 1:
            for b in range(batch_size):
                if self.is3d:
                    ...
                    y_poisson[b, :, :, :] = (
                        np.random.poisson(y_att[b, :, :, :] * scale_factor)
                        / scale_factor
                    )
                else:
                    scale_factor = counts[b] * truesFraction / y_att[b, :, :].sum()
                    y_poisson[b, :, :] = (
                        np.random.poisson(y_att[b, :, :] * scale_factor) / scale_factor
                    )
        else:
        ...
        if randomsFraction != 0:
            ...
            if batch_size > 1:
                for b in range(batch_size):
                    if self.is3d:
                        scale_factor_randoms = (
                            counts[b] * randomsFraction / r_poisson[b, :, :, :].sum()
                        )
                        r_poisson[b, :, :, :] = (
                            np.random.poisson(
                                r_poisson[b, :, :, :] * scale_factor_randoms
                            )
                            / scale_factor_randoms
                        )
                    else:
                        ...
                        r_poisson[b, :, :] = (
                            np.random.poisson(r_poisson[b, :, :] * scale_factor_randoms)
                            / scale_factor_randoms
                        )
            else:
                scale_factor_randoms = counts * randomsFraction / r_poisson.sum()
                r_poisson = (
                    np.random.poisson(r_poisson * scale_factor_randoms)
                    / scale_factor_randoms
                )
        else:
            Randoms = 0
    ...
    def fwd_subprocess(
        ...
        subprocess.run(
            [self.engine.binPath + self.engine.bar + "project", config_flname],
    ...
    def forwardProjectBatch3D(self, img, nsubs=1, subsetIndex=0, psf=0):
        ...
        sino_shape = self.sinogram.shape
        sinoOut = np.zeros(
            (batch_size, sino_shape[0], sino_shape[1], sino_shape[2]), dtype="float32"
        )
        out_sino_flname = []
        sample_sino_flname = []
        input_img_flname = []
        ...
        for b in range(num_temp):
            out_sino_flname.append(
                self.engine.temPath[b] + self.engine.bar + "out_sino"
            )
            sample_sino_flname.append(
                self.engine.temPath[b] + self.engine.bar + "sample_sino"
            )
            input_img_flname.append(
                self.engine.temPath[b] + self.engine.bar + "input_img"
            )
            config_flname.append(
                self.engine.temPath[b] + self.engine.bar + "fwdproj.par"
            )
            if not os.path.exists(sample_sino_flname[b] + ".h33"):
                self.write_to_apirl(sample_sino_flname[b], sino_mode=True)
            self.createConfigFile(
                config_flname[b],
                input_img_flname[b] + ".h33",
                sample_sino_flname[b] + ".h33",
                out_sino_flname[b],
                self.engine.gpu,
                True,
                nsubs,
    ...
    def bkd_subprocess(
        ...
        subprocess.run(
            [self.engine.binPath + self.engine.bar + "backproject", config_flname],
            stdout=subprocess.PIPE,
        )
        img = (
            np.fromfile(out_img_flname + ".i33", dtype="float32")
            .reshape(matrix_size, order="F")
            .transpose(1, 0, 2)
        )
        img[np.isnan(img)] = 0
        if np.any(psf != 0):
    ...
    def backProjectBatch3D(self, sino, nsubs=1, subsetIndex=0, psf=0):
        ...
        matrix_size = self.image.matrixSize
        imgOut = np.zeros(
            (batch_size, matrix_size[0], matrix_size[1], matrix_size[2]),
            dtype="float32",
        )
        out_img_flname = []
        sample_img_flname = []
        ...
        for b in range(num_temp):
            ...
            sample_img_flname.append(
                self.engine.temPath[b] + self.engine.bar + "sample_img"
            )
            input_sino_flname.append(
                self.engine.temPath[b] + self.engine.bar + "input_sino"
            )
            config_flname.append(
                self.engine.temPath[b] + self.engine.bar + "backproj.par"
            )
            self.createConfigFile(
                config_flname[b],
                input_sino_flname[b] + ".h33",
                sample_img_flname[b] + ".h33",
                out_img_flname[b],
                self.engine.gpu,
                False,
                nsubs,
            ...
            if not os.path.exists(sample_img_flname[b] + ".h33"):
                self.write_to_apirl(
                    sample_img_flname[b], np.zeros(matrix_size, dtype="float32")
                )
    ...
    def iSensImageBatch3D(self, AN=None, nsubs=1, psf=0):
        ...
        sensImgOut = np.zeros(
            (
                batch_size,
                nsubs,
                self.image.matrixSize[0],
                self.image.matrixSize[1],
                self.image.matrixSize[2],
            ),
            dtype="float32",
        )

        for i in range(nsubs):
            sensImgOut[:, i, :, :, :] = self.backProjectBatch3D(
                AN, nsubs=nsubs, subsetIndex=i, psf=psf
            )
    ...
    def forwardDivideBackwardBatch3D(
        self, img, prompts, RS, AN, nsubs, subsetIndex, psf
    ):
        if RS is None:
            RS = 0
        y = (
            self.forwardProjectBatch3D(
                img, nsubs=nsubs, subsetIndex=subsetIndex, psf=psf
            )
            + RS
            + 1e-4
        )
        out = self.backProjectBatch3D(
            prompts / y, nsubs=nsubs, subsetIndex=subsetIndex, psf=psf
        )
        out[np.isinf(out)] = 0
        return out
    ...
    def OSEM3D(
        ...
        if AN is None:
            if batch_size > 1:
                sino_shape = [
                    batch_size,
                    self.sinogram.shape[0],
                    self.sinogram.shape[1],
                    self.sinogram.shape[2],
                ]
            else:
                sino_shape = self.sinogram.shape
        ...
        for n in range(niter):
            ...
            for m in range(nsubs):
                ...
                img = (
                    img
                    * self.forwardDivideBackwardBatch3D(
                        img, prompts, RS, AN, nsubs, m, psf
                    )
                    * iSenImg_m
                )
    ...
    def forwardDivideSubtractBackwardBatch3D(
        self, img, prompts, RS, AN, nsubs, subsetIndex, psf
    ):
        y = (
            self.forwardProjectBatch3D(
        ...
        out = self.backProjectBatch3D(
            ((prompts / y) - 1.0), nsubs=nsubs, subsetIndex=subsetIndex, psf=psf
        )
        out[np.isinf(out)] = 0
        return out
        ...
        for n in range(niter):
            for m in range(nsubs):
                if batch_size == 1:
                    ...
                    for b in range(batch_size):
                        gamma = beta[b] * wj
                        img_reg = img[b, :, :, :] - 1 / 2 * wj_ * prior.GradT(
                            W * prior.Grad(img[b, :, :, :])
                        )
                        img[b, :, :, :] = (
                            2
                            * img_em[b, :, :, :]
                            / (
                                ...
                                + np.sqrt(
                                    (1 - gamma * iS[b, :, :, :] * img_reg) ** 2
                                    + 4 * gamma * iS[b, :, :, :] * img_em[b, :, :, :]
                                )
                            )
                        )
                if display:
                    (
                        plt.imshow(
                            (self.crop_img(img, 0.4))[:, :, 50], cmap="gist_heat"
                        ),
                        plt.pause(0.1),
    ...
    def removeSampleFiles(self):
        ...
        temPath = (
            self.engine.temPath
            if type(self.engine.temPath) == str
            else self.engine.temPath[0]
        )
        dirname = os.path.dirname(temPath)
        folders = os.listdir(dirname)
        for f in folders:
            fn = dirname + self.engine.bar + f + self.engine.bar
            if os.path.exists(fn + "sample_img.h33"):
                os.remove(fn + "sample_img.h33")
            if os.path.exists(fn + "sample_img.i33"):
                os.remove(fn + "sample_img.i33")
            if os.path.exists(fn + "sample_sino.h33"):
                os.remove(fn + "sample_sino.h33")
            if os.path.exists(fn + "sample_sino.i33"):
                os.remove(fn + "sample_sino.i33")
        return
    ...
    ******************************************************************************************************************
    ...
    def crop_sino(self, sino):
        if self.sinogram.radialBinCropfactor != 0:
            i = (
                int(
                    np.ceil(sino.shape[0] * self.sinogram.radialBinCropfactor / 2.0) * 2
                )
                // 2
            )
    ...
    def read_sino(self, flname, num_planes=None, dtype="float32"):
        ...
        sino_size = [
            self.sinogram.nRadialBins_orig,
            self.sinogram.nAngularBins,
            num_planes,
        ]
        return np.fromfile(flname, dtype, np.prod(sino_size)).reshape(
            sino_size, order="F"
        )

    def get_e7sino(self, path):
        prompts = self.crop_sino(self.read_sino(path + self.engine.bar + "emis_00.s"))
        randoms = self.crop_sino(
            self.read_sino(path + self.engine.bar + "smoothed_rand_00.s")
        )
        ncf = (
            self.crop_sino(self.read_sino(path + self.engine.bar + "norm3d_00.a"))
            * self.get_gaps()
        )
        ...
        scatters = self.iSSRB(
            self.crop_sino(
                self.read_sino(
                    path + self.engine.bar + "scatter_estim2d_000000.s",
                    2 * self.scanner.nCrystalRings - 1,
                )
            )
        )
