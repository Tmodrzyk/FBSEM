# -*- coding: utf-8 -*-
"""
Created on Sun Feb 10 11:53:22 2019
...
class Prior(object):
        ...
        for x in xidx:
            ...
            for y in yidx:
                ...
                for z in zidx:
                    ...
                    N[:, l] = (
                        (Xnew + (Ynew) * n + (Znew) * n * m).reshape(-1, 1).flatten("F")
                    )
    ...
    def imCrop(self, img=None):
        if np.any(self.imageCropFactor):
            ...
            imageSizeCrop = [
                np.arange(I, self.imageSize[0] - I).shape[0],
            ...
            if img is not None:
                if self.is3D:
                    img = img[
                        I : self.imageSize[0] - I,
                        J : self.imageSize[1] - J,
                        K : self.imageSize[2] - K,
                    ]
                else:
    ...
    def gaussianWeights(self, img, sigma):
        w = (
            ...
            * np.exp(-0.5 * self.Grad(img) ** 2 / sigma**2)
        )
    ...
    def BowshserWeights(self, img, b):
        if b > self.nS:
            raise ValueError(
                "Number of most similar voxels must be smaller than number of voxels per neighbourhood"
            )
