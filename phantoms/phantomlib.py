...
def random_lesion(img, num_lesions, lesion_size_mm, voxel_radious_mm=0.5):
    ...
    r = lesion_size_mm[0] / voxel_radious_mm + (
        lesion_size_mm[1] - lesion_size_mm[0]
    ) / voxel_radious_mm ** np.random.rand(num_lesions)
    lesions = np.zeros((img.shape[0], img.shape[1], img.shape[2], num_lesions))

    for le in range(num_lesions):
        lesions[:, :, :, le] = (
            ((xx - i[le]) ** 2 + (yy - j[le]) ** 2 + (zz - k[le]) ** 2) < r[le] ** 2
        ).astype("float")
...
def regrid(img, voxel_size, new_voxel_size, method="linear"):
    ...
    new_shape = [
        int(np.round(img.shape[k] * voxel_size[k] / new_voxel_size[k]))
    ...
    if np.ndim(img) == 3:
        ...
        x, y, z = [
            np.linspace(0, voxel_size[k] * (img.shape[k] - 1), new_shape[k])
            for k in range(3)
        ]
        ...
        if Angle[a] != 0:
            if np.ndim(img) == 3:
                for i in range(img.shape[2]):
                    imgr[a, :, :, i] = rotate(
                        img[:, :, i], Angle[a], reshape=False, order=1
                    )
            else:
