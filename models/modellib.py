    ...
    def __init__(self, depth, num_kernels, kernel_size, in_channels, is3d):
        ...
        if is3d:
            ...
            for _ in range(depth - 2):
                layers.append(
                    nn.Conv3d(num_kernels, num_kernels, kernel_size, padding=1)
                )
                layers.append(nn.BatchNorm3d(num_kernels))
                layers.append(nn.ReLU(inplace=True))
            ...
            for _ in range(depth - 2):
                layers.append(
                    nn.Conv2d(num_kernels, num_kernels, kernel_size, padding=1)
                )
                layers.append(nn.BatchNorm2d(num_kernels))
                layers.append(nn.ReLU(inplace=True))
...
class FBSEMnet_v3(nn.Module):
    def __init__(
        ...
        in_channels=1,
        is3d=False,
        reg_ccn_model="resUnit",
    ):
        super(FBSEMnet_v3, self).__init__()
        if reg_ccn_model.lower() == "resUnit".lower():
            self.regularize = ResUnit_v2(
                depth, num_kernels, kernel_size, in_channels, is3d
            )
        else:
    ...
    def forward(
        ...
        if self.is3d:
            ...
            reShape = lambda x: x.reshape(
                [batch_size, matrixSize[0], matrixSize[1]], order="F"
            )
            Flatten = lambda x: x.reshape(
                [batch_size, matrixSize[0] * matrixSize[1]], order="F"
            )
            toTorch = lambda x: zeroNanInfs(
                Crop(
                    torch.from_numpy(reShape(x))
                    .unsqueeze(1)
            ...
            if img is None:
                img = np.ones(
                    [batch_size, matrixSize[0] * matrixSize[1]], dtype="float32"
                )
            if batch_size == 1:
                if iSensImg.ndim == 2:
                    iSensImg = iSensImg[None].astype("float32")
        ...
        for i in range(niters):
            for s in range(nsubs):
                if self.is3d:
                    img_em = (
                        img
                        * PET.forwardDivideBackwardBatch3D(
                    ...
                    img_em = (
                        img
                        * PET.forwardDivideBackwardBatch2D(
                ...
                imgt = (
                    ...
                    * img_emt
                    / (
                        (1 - self.gamma * S * img_regt)
                        + torch.sqrt(
                            (1 - self.gamma * S * img_regt) ** 2
                            + 4 * self.gamma * S * img_emt
                        )
                    )
                )
                img = toNumpy(imgt)
    ...
    for e in range(g.epochs):
        ...
        for sinoLD, imgHD, AN, _, _, _, mrImg, _, _, index in train_loader:
            ...
            if g.do_validation:
                ...
                model.train()
                print(
                    f"Epoch: {e+1}/{g.epochs}, Validation loss: {valid_losses[e]:.3f}"
                )
            if ((g.save_from_epoch is not None) and (g.save_from_epoch <= e)) or e == (
                g.epochs - 1
            ):
                ...
                torch.save(
                    checkpoint, g.save_dir + g.model_name + "-epo-" + str(e) + ".pth"
                )
...
def fbsemInference(
    dl_model_flname, PET, sinoLD, AN, mrImg, niters=None, nsubs=None, device="cpu"
):
    toNumpy = lambda x: x.detach().cpu().numpy().astype("float32")
