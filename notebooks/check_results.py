...
img_idx = 25  # poor mrFBSEM performances
# img_idx = 56  # good mrFBSEM performances
# img_idx = 81  # bad pet-PnP-MM performances
# img_idx = 47
reference = np.load(
...
pnpmm_pet_recon = np.load(
    f"../tests/PNPMM-pet/20251119_110158/recons/recon_{str(img_idx).zfill(3)}.npy"
).squeeze()
...
eps = 1e-16
mask = reference_cropped > (0.01 * reference_cropped.max())
...
v = np.percentile(abs_vals, 95)  # robust symmetric range
...
for img in images_lc[1:]:
    nrmse = (
        torch.sqrt(
            mse(
                torch.from_numpy(img[mask]).unsqueeze(0).unsqueeze(0),
                torch.from_numpy(reference_cropped[mask]).unsqueeze(0).unsqueeze(0),
            )
        )
        / torch.norm(
            torch.from_numpy(reference_cropped[mask]).unsqueeze(0).unsqueeze(0)
